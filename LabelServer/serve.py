...
def setLocales():
    ...
    fr = gettext.translation('spifedit', os.path.join(os.getcwd(), "locales"), ['fr'])
    fr.install()
    _ = fr.gettext # french
...
def loadConfig():
    ...
    if os.path.exists(app.config['CONFIG_FILE']): 
        ...
        if configDict.has_key('spif config'): app.config['CONFIG_DIRECTORY'] = configDict['spif config']
            ...
            syslog.syslog(
                syslog.LOG_WARNING,
                _('spif_config not set in /etc/x841/spifedit.yml. Using {0}.').format(app.config['CONFIG_DIRECTORY']))
    else: 
        configDict = {}
...
def showDefault(): 
    syslog.syslog(syslog.LOG_INFO, _('Web app started'))
    myOid = configDir.files[0].oid
    selected = configDir.files[0].oid
    syslog.syslog(syslog.LOG_DEBUG, _('showDefault oid {0} - selected {1}').format(str(myOid),selected))
...
def showOid(oid):
    syslog.syslog(syslog.LOG_INFO, _('showOid : show classifications for oid {0}').format(oid))
...
def changeClassifications(oid,classif):
    syslog.syslog(syslog.LOG_INFO,_('Changing classification {1} for oid {0}').format(oid, classif))
    ...
    syslog.syslog(syslog.LOG_DEBUG, _('Hierarchy set from {0} to {1}, LACV set from {2} to {3}').format(oldHierarchy, classifObject.getHierarchy(),oldLacv, classifObject.getLacv()))
...
def addClassifications(oid):
    ...
    syslog.syslog(syslog.LOG_INFO, _("Adding classification {1} for oid {0}").format(oid, cname))
    
    # Fill a dictionary
    myDict = {}
    try: myDict['name'] = request.form['name']
    except KeyError: raise KeyError(_("Error : unnamed classification"))
    try: myDict['hierarchy'] = request.form['hierarchy']
    except KeyError: raise KeyError(_("Error : no hierarchy in request"))
    try: myDict['lacv'] = request.form['lacv']
    except KeyError: raise KeyError(_("Error : no lacv in request"))

    # Find SPIF & add classification
    confFile = configDir.getConfigFile(oid)
    if confFile == None: return (Response(_("Unknown SPIF/object id"),code=404)) 
    
    classif = Classification(myDict)
    confFile.classifications.append(classif)
    syslog.syslog(syslog.LOG_DEBUG, _('Added classification {0}/{1} (h:{2},l:{3})').format(oid,classif.getName(),classif.getHierarchy(),classif.getLacv()))
...
def delClassifications(oid,classif):
    syslog.syslog(syslog.LOG_INFO, _("Deleting classification {1} for oid {0}").format(oid, classif))
    
    confFile = configDir.getConfigFile(oid)
    if confFile == None: return (Response(_("Unknown SPIF/object id"),code=404))
    confFile.delClassification(classif)
    syslog.syslog(syslog.LOG_DEBUG, _('Deleted classification {0}').format(classif))
